...
<template>
  <div @mouseenter="mouseHover" @mouseleave="mouseRemoveHover" :class="['flex flex-col md:flex-row md:gap-0 md:justify-center md:items-start', align === 'right' ? 'md:flex-row-reverse' : '']">
    <div class="card-group-default__media overflow-hidden aspect-[4/3] md:h-[29.53vw] md:w-[39.31vw] [&_img]:md:aspect-[4/3] [&_img]:md:!h-[unset] [&_img]:md:!w-[unset] lg:h-[22.9vw] lg:w-[30.62vw]">
      <ClientOnly>
    ...
    <div :class="['flex flex-col justify-center gap-[24px] py-[24px] md:flex-1 md:items-start md:self-stretch md:py-[20px] xl:py-[20px] xl:justify-center', align === 'right' ? 'md:pr-[40px] md:pl-[20px] xl:pr-[60px]' : 'md:pl-[40px] md:pr-[20px] xl:pl-[60px]']">
      <div v-if="title || tag" class="flex flex-col gap-[10px] xl:gap-[20px]">
        <BodyText v-if="tag" wrapper="label" type="strong"> {{ tag }} </BodyText>
        <BodyText v-if="title" wrapper="h2" class="medium"> {{ title }} </BodyText>
      </div>
